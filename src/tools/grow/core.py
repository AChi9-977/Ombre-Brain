"""
========================================
tools/grow/core.py — grow 长内容主路径（digest + merge）+ 预拆分模式
========================================

长内容（≥30 字）走这里。先调 dehydrator.digest 把整段拆成 2~6 条
事件项，每条独立尝试 merge_or_create。
预拆分模式（grow_items）：上层 AI 已拆好 N 条最终正文 → 逐字入库，跳过 digest。
...
对外暴露：grow_core(content, trigger_date) → str
         grow_items(items) → str
========================================
"""
    ...
    for content_str in clean:
        try:
            ...
            if is_merged:
                ...
                results.append(f"📝{result_name}")
                created += 1
                asyncio.create_task(post_write_relations(result_name, content_str))
        except Exception as e:
