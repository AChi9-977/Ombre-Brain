...
- 每桶一行：ID + 名称 + 主题 + 情感坐标 + 重要度 + 更新时间，按重要度降序
...
async def surface_catalog(domain_filter: list[str] | None = None) -> str:
    ...
    for b in buckets:
        ...
        try:
            ...
            arousal = 0.3
        updated = str(
            meta.get("updated_at") or meta.get("last_active") or meta.get("created") or "未知"
        )
        # Keep the historical ``name | domain | importance`` prefix so older
        # clients/tests can parse it, then append the richer summary fields.
        line = (
            f"{pin_mark}{name} | {','.join(domains) or '未分类'} | {imp} | "
            f"ID:{b['id']} | 情感:V{valence:.1f}/A{arousal:.1f} | 更新:{updated}"
        )
        btype = meta.get("type")
