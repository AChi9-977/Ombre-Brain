...
- catalog 增加桶 ID、情感坐标和稳定更新时间，仍保持一桶一行且不返回正文。
- breath 检索默认普通桶上限改为 5，核心命中不占名额；精确字段/正文原词严格优先于纯语义，支持按 `updated_at` 日期过滤，并移除显式搜索中的随机漂浮。
- pulse 默认只列全部核心桶与权重最高的 15 个普通桶；dream 默认最近 5 个摘要，`detail_ids` 可取指定全文。
- trace 支持最多 50 个 ID 的安全批量元数据操作，以及普通动态桶之间的显式合并；合并源桶只归档，不物理删除。
