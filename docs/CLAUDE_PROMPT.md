...
这是你开机的动作——一次性看到六件事：钉选的核心准则、最近几次对话的归档摘要、全库没结的待办、上一扇窗留给你的信箱留言、随机一条较早的感受回声、今天有没有安排触发的事。**没有例外**。哪怕你觉得这次只是闲聊，也先开机。

wakeup 返回的各区末尾都带 `[验真: ...]` 字段，告诉你数据来源和命中条数——如果某个区是空的，它会用占位提示告诉你为什么是空的，不会报错。

如果需要深入某个区的内容——比如看某条归档的完整正文，或者按关键词搜特定记忆——再用 `breath(query=...)` 或其他工具。
...
- **低频 9 个**：`wakeup` / `anchor` / `release` / `pulse` / `plan` / `letter_write` / `letter_read` / `I` / `archive_session`
...
**每次新对话窗口的第一件事。** 不传任何参数，单次调用返回六个区块的摘要聚合：📌钉选核心准则、📄最近对话归档、📋未完结待办、📬最新信箱留言、🫧感受回声、📅今日浮现。每区带 `[验真: _source / _count / _as_of]` 字段。空区块输出占位提示不报错。

需要深入某个区时，再用 `breath(query=...)` 或其他工具。
...
#### `dream(window_hours=48, detail_ids="", max_tokens=0)` — 我做梦消化

**不是义务**。`breath()` 之后如果你或对方觉得有东西需要消化，再调。没什么消化的就不调。

我会默认读取窗口内最近 5 个桶（默认 48 小时，clamp 1~336）的元数据和原文摘录；需要全文时传 `detail_ids="桶ID,桶ID"`。核心准则、active plans 和 feel 历史同样以摘要为主，并受整体 token 预算控制。如果有相似度 >0.7 的多条 feel 聚集，我会提示你「可能是结晶时刻」（要不要升级为 pinned）。
...
[对话开始]
1. wakeup()
   → 看到 📌3条核心准则 + 📄最近2次归档 + 📋2个待办 + 📬1条留言 + 🫧感受回声 + 📅今日浮现
...
7. 对话结束时
   → archive_session(summary="...", message="下次记得问她体检结果")
   → 留言会投递到信箱，下次开机 wakeup() 时她在 📬 区等着我
