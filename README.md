...
| `breath_search` | 按关键词 / 语义找记忆：默认最多 5 个普通桶，钉选/保护/固化命中不占名额；支持 `date_from` / `date_to` 按修改日期过滤，末尾提示未显示数量。精确字段与正文原词优先于纯语义命中。 |
| `breath_advanced` | `breath` 的完整参数版：`catalog=True` 目录模式（ID、名称、主题、情感、重要度、更新时间；无正文、0 LLM）、`tags`、`importance_min`、日期、情感坐标和 token 预算等精细控制。 |
...
| `trace` | 唯一的元数据写入口：支持逗号分隔的批量 ID（最多 50 个、安全字段限定），也可用 `merge=目标ID` 显式合并普通桶；源桶只归档、不物理删除。 |
| `dream` | 默认返回最近 5 个桶的元数据与原文摘录；`detail_ids` 指定的桶返回全文，辅助区块受 `max_tokens` 控制。**不是义务**，需要消化时再调。 |

### 低频 7 个

| 工具 | 一句话 |
|---|---|
| `pulse` | 自检：默认列全部核心桶 + 权重最高的 15 个普通桶，统计放末尾；`show_all=True` 才列全部。 |
